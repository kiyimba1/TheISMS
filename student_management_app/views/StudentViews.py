...
from student_management_app.models import Students, Clss, Subjects, CustomUser, Attendance, AttendanceReport, \
    LeaveReportStudent, FeedBackStudent, NotificationStudent, StudentResult, OnlineClassRoom, Term, Payments
...
def student_home(request):
    ...
    course = Clss.objects.get(id=student_obj.clss_id.id)
    subjects = Subjects.objects.filter(clss_id=course).count()
    subjects_data = Subjects.objects.filter(clss_id=course)
    session_obj = Term.objects.get(id=student_obj.term_id.id)
...
def join_class_room(request, subject_id, session_year_id):
    ...
    if subjects.exists():
        session = Term.object.filter(id=session_year_obj.id)
        if session.exists():
            subject_obj = Subjects.objects.get(id=subject_id)
            course = Clss.objects.get(id=subject_obj.course_id.id)
            check_course = Students.objects.filter(
                id=request.user.id, course_id=course.id)
            if check_course.exists():
                session_check = Students.objects.filter(
                    id=request.user.id, session_year_id=session_year_obj.id)
                if session_check.exists():
                    ...
                    return render(request, "student_template/join_class_room_start.html",
                                  {"username": request.user.username, "password": onlineclass.room_pwd,
...
def student_view_attendance(request):
    ...
    course = student.clss_id
    subjects = Subjects.objects.filter(clss_id=course)
...
def student_view_attendance_post(request):
    ...
    stud_obj = Students.objects.get(id=user_object)
...
def student_apply_leave(request):
    staff_obj = Students.objects.get(id=request.user.id)
...
def student_feedback(request):
    staff_id = Students.objects.get(id=request.user.id)
...
def student_profile_save(request):
    if request.method != "POST":
        ...
        try:
            ...
            student = Students.objects.get(id=customuser)
...
def student_view_result(request):
    student = Students.objects.get(id=request.user.id)
    studentresult = StudentResult.objects.filter(student_id=student.id.id)
    return render(request, "student_template/student_result.html", {"studentresult": studentresult})


def student_view_payments(request):
    student = Students.objects.get(id=request.user.id)
    payments = Payments.objects.get(student_id=student.id)
    return render(request, "student_template/student_payment.html", {"payments": payments})
