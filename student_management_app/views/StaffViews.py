...
from student_management_app.models import Subjects, Term, Students, Attendance, AttendanceReport, \
    LeaveReportStaff, Staffs, FeedBackStaffs, CustomUser, Clss, NotificationStaffs, StudentResult, OnlineClassRoom
    ...
    try:
        ...
        if check_exist:
            ...
            result = StudentResult(student_id=student_obj, subject_id=subject_obj, subject_exam_marks=exam_marks,
                                   subject_assignment_marks=assignment_marks, term_id=term_obj, aggregate=aggregate,
                                   remark=remark, initails=staff_obj)
            result.save()
...
def start_live_classroom_process(request):
    ...
    if checks:
        ...
        staff_obj = Staffs.objects.get(admin=request.user.id)
        onlineClass = OnlineClassRoom(room_name=roomname, room_pwd=room_pwd, subject=subject_obj,
                                      session_years=session_obj, started_by=staff_obj, is_active=True)
        onlineClass.save()
